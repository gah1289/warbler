...
from models import db, connect_db, User, Message, bcrypt, Likes
...
def login():
    ...
    if form.validate_on_submit():   
        ...
        if not user_exists: 
            ...
            # site was crashing when I entered username that didn't exist
...
def list_users():
    ...
    if not search:
        ...
        # change like to ilike to make searches case insensitive
...
def like_message(msg_id):
    '''Click like button and add to likes'''
    message=Message.query.get_or_404(msg_id)
    if message.user_id != g.user.id:
        new_like=Likes(user_id=g.user.id, message_id=msg_id)
        db.session.add(new_like)
        db.session.commit()
        flash('Liked warble!', 'info')
        return redirect('/')
    else:
        flash(f"You can't like your own tweet", "warning")
        return redirect('/users/add-like/<int:msg_id>')

@app.route('/users/remove-like/<int:msg_id>', methods=["POST"])

@app.route('/users/remove-like/<int:isg_id>')
def unlike_message(msg_id):
    '''Click star button of liked tweet and remove from likes''' 
    like=Likes.query.filter_by(user_id=g.user.id, message_id=msg_id).one() 
    db.session.delete(like)
    db.session.commit()
    flash('Removed warble from likes', 'info') 
    return redirect('/')

@app.route('/users/<int:user_id>/likes')
def show_user_likes(user_id):
    liked_msgs=Likes.query.filter_by(user_id=user_id).all()
    liked_msg_ids=[]
    for liked_id in liked_msgs:
        liked_msg_ids.append(liked_id.message_id)      
    messages = (Message
                    .query
...
def homepage():
    ...
    if g.user:
        ...
        # generate list of ids for users being followed     
        messages = (Message
                    .query
        ...
        for like in g.user.likes:
            likes.append(like.id)
        # return a list of likes so jinja can filter through them in home.html 
